...
async function main() {
  ...
  const implementation = await prisma.wikiPage.create({
    data: {
      ...
      // Des TITRES, sans quoi le sommaire latéral n'aurait rien à montrer et
      // l'on croirait la fonctionnalité absente.
      content: [
        "Les pièces jointes ne transitent jamais par la base : seules leurs métadonnées y figurent.",
        "",
        "## Où vivent les fichiers",
        "",
        "Un stockage compatible S3 (MinIO en local, S3 en production). Le dossier `.uploads` sert de repli quand rien n'est configuré - pratique en développement, à proscrire en production où le disque n'est pas persistant.",
        "",
        ...
        "",
        "Une page de wiki cite ses images par `/api/wiki-files/<id>`, jamais par l'URL signée : celle-ci expire en quelques minutes, et le texte enregistré afficherait des images cassées pour toujours.",
        "",
        "## Limites",
        "",
        "20 Mo par fichier. Les types exécutables sont refusés au dépôt, et le contrôle est rejoué à l'enregistrement.",
      ].join("\n"),
      authorId: admin.id,
    },
  });
